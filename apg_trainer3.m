function [s_curr,s_hat,result,A_h,B_h] = apg_trainer3(Mtb,para,b,s_hat,btest)      
...
for s_i=1:para.N 
    ...
    if strcmp( para.verbose, 'all')||strcmp( para.verbose, 'outer')
       ...
       [recX] = rec_fre(vd_hat,z_hat_si,para);
        ...
        ps_test = my_psnr(temp_btest,reCX,para);
        fprintf('Y: no.iter: %d, psnr: %2.2f, nnz: %i, cr: %.2f\t', iterZ,ps,NNZ,CR)
        
        result.PSNR_test_i(s_i) = ps_test;
...
result.PSNR_test_i = gather(result.PSNR_test_i);

result.PSNR_test = mean(result.PSNR_test_i);
